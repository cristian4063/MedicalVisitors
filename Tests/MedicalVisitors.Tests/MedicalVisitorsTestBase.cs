...
namespace MedicalVisitors.Tests
{
    public abstract class MedicalVisitorsTestBase : AbpIntegratedTestBase<MedicalVisitorsTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MedicalVisitorsTestBase()
        {
        ...
        protected void UsingDbContext(Action<MedicalVisitorsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MedicalVisitorsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MedicalVisitorsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MedicalVisitorsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MedicalVisitorsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MedicalVisitorsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MedicalVisitorsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MedicalVisitorsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MedicalVisitorsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MedicalVisitorsDbContext, Task<T>> func)
        {
            T result;
